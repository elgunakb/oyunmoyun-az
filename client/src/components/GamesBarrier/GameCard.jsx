...
const GameCard = React.memo(function GameCard({ title, image, Icon, onClick }) {
  ...
  return (
    <AccessibleButton
      onClick={onClick}
      className="group/card relative w-full cursor-pointer select-none rounded-2xl border-2 border-gray-600/50 bg-linear-to-br from-gray-800/50 to-gray-900/50 transition-transform duration-300 hover:scale-105 hover:border-gray-500 hover:shadow-2xl hover:shadow-gray-500/10 text-left"
    ...
    >
      ...
      <div className="p-6 flex flex-col items-center justify-between w-full aspect-3/2">
        ...
        <div className="flex items-center justify-center w-full min-h-[2.2rem] px-2">
          <p className="font-semibold text-center text-base leading-tight wrap-break-word tracking-wide text-white">
            {title}
          </p>
        </div>
      </div>

      <div className="pointer-events-none absolute inset-0 bg-linear-to-t from-blue-600/25 to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity duration-300" />
    </AccessibleButton>
  );
});
